...
export default {
  ...
  methods: {
    ...
    sortBy(sortKey) {
      ...
      this.$store.commit('saveTriggers', _.orderBy(this.triggers, sortKey === 'creator.username' ? [user => user.creator.username.toLowerCase()] : sortKey, this.reverse ? 'desc' : 'asc'))
    },
    filterBy(list, value) {
  ...
  created() {
    ...
    if (this.triggersTotal == 0) {
      axios.get(`https://mitchdev.net/bot/triggers`)
      .then(response => {
        if (this.isMobile) {
